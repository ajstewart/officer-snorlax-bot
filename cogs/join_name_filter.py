...
class JoinNameFilter(commands.GroupCog, name="join-name-filter"):
    """Cog for banning a user that joins the server that matches a specified name."""
    ...
    @app_commands.command(
        name="activate", description="Turns on the 'join name' filter."
    )
    @app_commands.default_permissions(administrator=True)
    ...
    @app_commands.command(
        name="deactivate", description="Turns off the 'join name' filter."
    )
    @app_commands.default_permissions(administrator=True)
    @app_commands.check(snorlax_checks.interaction_check_bot)
    @app_commands.check(snorlax_checks.check_admin_channel)
    ...
    async def on_member_join(self, member: Member) -> None:
        """Checks for and then bans any new members that match a name provided.
                ...
                if pattern.lower() in member.name.lower():
                    try:
                        ...
                        if log_channel_id != -1:
                            ...
                            embed = ban_log_embed(
                                member, tz, f"Name filter matched with '{pattern}'."
                            )
...
async def setup(bot: commands.bot) -> None:
    ...
    if bot.test_guild is not None:
        await bot.add_cog(JoinNameFilter(bot), guild=discord.Object(id=bot.test_guild))
    else:
