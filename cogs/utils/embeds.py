...
def get_friend_channels_embed(friend_db: pd.DataFrame) -> Embed:
    ...
    """
    embed = Embed(title="Friend Code Channels", color=2061822)
...
def get_open_embed(
    ...
    Args:
        ...
        time_format_fill: The string time channel mention, or 'Unavailable' if the
            time channel is not configured.
    ...
    embed = Embed(
        title="✅  Channel Open!", description=base_open_message, color=3066993
    )

    close_time_str = datetime.datetime.strptime(close, "%H:%M").strftime("%I:%M %p")

    embed.add_field(
        name="Scheduled Close Time", value=f"{close_time_str} {now.tzname()}"
    )
...
def get_close_embed(
    ...
    embed = Embed(
        title="️⛔  Channel Closed!", description=base_close_message, color=15158332
    )

    open_time_str = datetime.datetime.strptime(open, "%H:%M").strftime("%I:%M %p")

    embed.add_field(name="Scheduled Open Time", value=f"{open_time_str} {now.tzname()}")
...
def get_warning_embed(
    ...
    """Produces the warning embed for channel closure.

    Args:
        close: The string representation of the future closing time, e.g. '12:00'.
        client_user: The bot user object.
        time_format_fill: The string time channel mention, or 'Unavailable' if the time
            channel is not configured.
        dynamic: Whether dynamic mode is activated on the schedule (True) or
            not (False).
    ...
    """
    embed = Embed(title="️⚠️  Snorlax is approaching!", color=15105570)
    ...
    embed.set_footer(
        text="Current time updates every 10 min.", icon_url=client_user.display_avatar
    )
...
def get_schedule_overwrites_embed(
    ...
    if len(roles_deny) > 0:
        ...
        embed.add_field(
            name="❌ Roles never able to send messages:", value=deny_value, inline=False
        )

    embed.add_field(
        name="How to fix?",
        value=(
            "Change the 'send messages' permission to neutral so it follows the"
            " @everybody role."
        ),
    )
...
def get_schedule_overwrites_embed_all_ok(channel: discord.TextChannel) -> Embed:
    ...
    embed = Embed(
        ...
        description=f"Any schedule in {channel.mention} will apply to all roles!",
        color=3066993,
        timestamp=utcnow(),
    )
...
def get_admin_channel_embed(admin_channel) -> Embed:
    ...
    embed = Embed(
        title="️Command cannot be used here!", description=description, color=15158332
    )
...
def get_message_embed(msg: str, msg_type: str, title: Optional[str] = None) -> Embed:
    ...
    if msg_type not in msg_colors:
        raise ValueError(
            f"Message type '{msg_type}' is not valid! (valid:"
            f" {', '.join(msg_colors.keys())})"
        )

    embed = Embed(color=msg_colors[msg_type], description=msg)
